...
export default function PasoResultAsegurados({ resultado, isCopied, onCopiarReporte, onDescargarErrores }: Props) {
    ...
    <div className="flex flex-col gap-6 py-2 animate-in fade-in slide-in-from-bottom-4 duration-300 max-w-3xl mx-auto">
      <div className="grid grid-cols-2 gap-4 w-full">
        <div className="bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-800/30 rounded-2xl p-5 text-center shadow-sm">
          ...
          <p className="text-xs text-emerald-700 dark:text-emerald-500 mt-1 uppercase font-bold tracking-wider">Clientes Importados</p>
        </div>
        <div className={`rounded-2xl p-5 text-center border shadow-sm ${
          ...
          <p className={`text-xs mt-1 uppercase font-bold tracking-wider ${resultado.salteados > 0 ? "text-red-700 dark:text-red-500" : "text-gray-400"}`}>Rechazados / Duplicados</p>
        </div>
      </div>
          ...
          {resultado.reporte?.map((item: any, idx: number) => (
            <div key={idx} className={`text-xs p-2.5 rounded-xl flex items-start gap-3 border ${
              ...
              <div className="flex flex-col gap-0.5">
                <p><strong className="font-mono text-[10px] bg-white/60 dark:bg-black/20 px-1.5 py-0.5 rounded mr-1">Fila {item.fila}</strong> {item.cliente} (DNI: {item.dni})</p>
