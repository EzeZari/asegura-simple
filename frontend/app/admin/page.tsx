...
export default function AdminDashboard() {
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <div className="animate-pulse flex flex-col items-center gap-4">
          <ShieldCheck size={48} className="text-indigo-500 opacity-50" />
          <p className="text-slate-400 font-medium tracking-widest uppercase text-sm">Cargando Sistema Maestro...</p>
        </div>
      </div>
    );
  }
  ...
  const planesOptions = [
    { id: "GRATUITO", nombre: "Gratuito", icon: <CreditCard size={20} />, color: "text-slate-400", bg: "bg-slate-400/10", border: "border-slate-400/20" },
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 selection:bg-indigo-500/30 font-sans relative">
      ...
      <header className="border-b border-slate-800 bg-slate-900/50 backdrop-blur-xl sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="bg-indigo-500/10 p-2.5 rounded-xl border border-indigo-500/20 text-indigo-400 shadow-inner">
              <ShieldCheck size={24} />
            </div>
            <div>
              <h1 className="text-xl font-black text-white tracking-tight">Backoffice</h1>
              <p className="text-xs text-indigo-400 font-bold tracking-wider uppercase">Nivel: Super Administrador</p>
            </div>
          </div>
          <button onClick={handleLogout} className="flex items-center gap-2 text-sm font-bold text-slate-400 hover:text-red-400 bg-slate-800/50 hover:bg-red-500/10 px-4 py-2.5 rounded-xl transition-all border border-transparent hover:border-red-500/20">
      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h2 className="text-2xl font-bold text-white flex items-center gap-2"><Users className="text-slate-500" /> Cuentas Registradas</h2>
            <p className="text-slate-400 mt-1">Radiografía completa de usuarios, roles y suscripciones del sistema.</p>
          </div>
          <div className="bg-slate-900 border border-slate-800 px-4 py-2 rounded-xl flex items-center gap-3 shadow-lg">
            <span className="text-sm font-medium text-slate-400">Total Usuarios:</span>
            <span className="text-lg font-black text-indigo-400">{agencias.length}</span>
          </div>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-950/50 text-slate-400 text-xs uppercase tracking-wider">
                  <th className="p-5 font-bold border-b border-slate-800">ID</th>
                  <th className="p-5 font-bold border-b border-slate-800">Usuario y Jerarquía</th>
                  <th className="p-5 font-bold border-b border-slate-800">Contacto</th>
                  <th className="p-5 font-bold border-b border-slate-800">Plan Actual</th>
                  <th className="p-5 font-bold border-b border-slate-800 text-right">Acciones</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800/50">
                {agencias.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="p-10 text-center text-slate-500 font-medium">No hay cuentas registradas.</td>
                  </tr>
                ) : (
                  agencias.map((agencia) => {
                    ...
                    const renderRolBadge = (usuario: any) => {
                      ...
                      if (rol.includes('ADMIN') || rol.includes('DUEÑO')) {
                        return <span className="bg-indigo-500/20 text-indigo-400 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border border-indigo-500/20">Dueño / Admin</span>;
                      }
                      ...
                      if (rol.includes('LECTOR')) {
                        return <span className="bg-slate-500/20 text-slate-400 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border border-slate-500/20">Lector</span>;
                      }
                      
                      return <span className="bg-slate-500/20 text-slate-400 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border border-slate-500/20">{rol}</span>;
                    };

                    return (
                      <tr key={agencia.id} className="hover:bg-slate-800/20 transition-colors">
                        <td className="p-5 text-sm text-slate-500 font-mono">#{agencia.id}</td>
                        <td className="p-5">
                          ...
                          {agencia.jefe && (
                            <div className="text-[11px] text-slate-400 mt-1 bg-slate-800/50 inline-block px-2 py-1 rounded-md">
                              Equipo de: <span className="font-bold text-slate-300">{agencia.jefe.nombre}</span>
                            </div>
                          )}
                        </td>
                        <td className="p-5">
                          <div className="flex flex-col gap-1.5">
                            <span className="flex items-center gap-2 text-sm text-slate-300"><Mail size={14} className="text-slate-500"/> {agencia.email}</span>
                            {agencia.telefono && <span className="flex items-center gap-2 text-xs text-slate-500"><Phone size={12}/> {agencia.telefono}</span>}
                          </div>
                        </td>
                        <td className="p-5">
                          {/* Si es empleado, el plan lo hereda del dueño */}
                          {agencia.jefeId ? (
                             <span className="text-xs font-medium text-slate-500 italic">Heredado del Dueño</span>
                          ) : (
                        ...
                        <td className="p-5 text-right">
                          ...
                          {agencia.jefeId ? (
                            ...
                            <button 
                              ...
                              className="text-sm font-bold text-indigo-400 hover:text-indigo-300 transition-colors px-4 py-2 bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/20 rounded-xl"
                            >
                              Modificar Plan
                            </button>
                          )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-slate-900 border border-slate-800 w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="px-6 py-4 border-b border-slate-800 flex items-center justify-between bg-slate-900/50">
              ...
              <button onClick={() => setModalOpen(false)} className="text-slate-500 hover:text-white transition-colors bg-slate-800/50 hover:bg-slate-800 p-2 rounded-full">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="p-6">
              <div className="mb-6">
                <p className="text-sm text-slate-400">Seleccioná el nuevo nivel de acceso para:</p>
                <p className="text-lg font-black text-white mt-1">{agenciaSeleccionada.nombre}</p>
                <p className="text-xs text-slate-500">{agenciaSeleccionada.email}</p>
              </div>
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <span className="block text-xs text-slate-500 mt-0.5">Nivel de acceso al sistema</span>
                      </div>
                    </div>
                    {planSeleccionado === plan.id && (
            ...
            <div className="px-6 py-4 border-t border-slate-800 bg-slate-950/50 flex justify-end gap-3">
              ...
              <button 
                ...
                disabled={isUpdating}
                className="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-bold rounded-xl shadow-lg shadow-indigo-900/20 transition-all flex items-center gap-2 disabled:opacity-50"
              >
